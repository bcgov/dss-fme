...
class FMEServerAPIJob:
    ...
    def __init__(self, app_config, secrect_config, key, output_dir, log):
        self.key = key
        self.job = FMEAPI.FmeApis(app_config[key + "_fme_server"], secrect_config[key + "_token"], log)
    ...
    def upload_fmw_prop(self, names, dir, overwrite, delete_act, exist_act, upload_act):
        ...
        if overwrite:
            ...
            if exist_act(*names):
                return
        response = upload_act(*names, prop_file)
        if not exist_act(*names):
            raise APIException(error)
        return response

    def upload_fmw(self, repo_name, fmw_name, dir, overwrite=False):
        del_func = lambda repo_name, fmw_name: self.job.delete_fmw(repo_name, fmw_name)
        exist_func = lambda repo_name, fmw_name: self.fmw_exists(repo_name, fmw_name)
        upload_func = lambda repo_name, fmw_name, file: self.job.upload_fmw(repo_name, fmw_name, file)
        return self.upload_fmw_prop([repo_name, fmw_name], dir, overwrite, del_func, exist_func, upload_func)
    ...
    def upload_fmw_resource(self, repo_name, fmw_name, resource_name, dir, overwrite=False):
        del_func = lambda repo_name, fmw_name, resource_name: self.job.delete_fmw_resource(repo_name, fmw_name,
                                                                                           resource_name)
        exist_func = lambda repo_name, fmw_name, resource_name: self.fmw_resource_exists(repo_name, fmw_name,
                                                                                         resource_name)
        upload_func = lambda repo_name, fmw_name, resource_name, file: self.job.upload_fmw_resource(repo_name, fmw_name,
                                                                                                    resource_name, file)
        return self.upload_fmw_prop([repo_name, fmw_name, resource_name], dir, overwrite, del_func, exist_func,
                                    upload_func)
